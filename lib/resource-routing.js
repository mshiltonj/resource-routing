...
var handler_wrapper_default = function(handler, req, res){
  console.log("DEFAULT WRAPPER");
  handler(req,res);
}
...
function build_route(app, method, url, controller_data, action){
  try {
    ...
    var indirect_handler = function(req, res){
      handler_wrapper(handler, req, res);
    }
...
var resource_routing = function(){
  ...
  this.expose_routing_table =  function(app, options){
    ...
    this.set_wrapper = set_wrapper;
    this.reset_wrapper = reset_wrapper;
